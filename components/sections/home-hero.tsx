...
export function HomeHero() {
  return (
    <section className="relative overflow-hidden">
      <div className="container grid min-h-[calc(100svh-4rem)] items-center gap-10 py-16 md:grid-cols-[1.05fr_0.95fr] md:py-24">
        <Reveal>
          ...
          <div className="relative mx-auto aspect-[4/3] w-full max-w-xl overflow-hidden rounded-lg border border-white/10 bg-card p-4 shadow-glow">
            ...
            <div className="relative grid h-full grid-rows-[auto_1fr_auto] gap-4">
              <div className="flex items-center justify-between rounded-md border border-white/10 bg-background/70 p-3">
                <span className="text-xs font-medium text-muted-foreground">Lead Engine Preview</span>
                <span className="rounded-full bg-primary/15 px-2 py-1 text-xs font-semibold text-primary">V1</span>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {["Strategy", "Build", "SEO", "Campaigns"].map((label, index) => (
                  <div key={label} className="rounded-md border border-white/10 bg-background/60 p-4">
                    <div className="mb-4 h-2 w-10 rounded-full bg-primary/70" />
                    <p className="text-sm font-semibold">{label}</p>
                    <p className="mt-2 text-xs leading-5 text-muted-foreground">Configurable placeholder module {index + 1}</p>
                  </div>
                ))}
              </div>
              <div className="rounded-md border border-white/10 bg-background/70 p-3 text-sm text-muted-foreground">
                Mobile-first, fast by default, measurable at every CTA.
              </div>
            </div>
          </div>
        </Reveal>
